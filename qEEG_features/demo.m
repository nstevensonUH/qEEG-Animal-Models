...
% Do filtering (50Hz notch, and 0.5Hz HPF at this stage)
A = size(dat);
for ii = 1:A(2);
    dum = dat(:,ii);
...
block_no2 = floor(ep/elen); fv = zeros(block_no2, 10+length(fbnd));
for kk1 = 1:block_no2
    r1 = (kk1-1)*elen+1; r2 = r1+elen-1;
    fval1 = zeros(2, 3); fval2 = fval1; fval4 = fval1; fval3 = zeros(2, length(fbnd));
    for kk2 = 1:ch_no
        dat1 = dat(r1:r2,kk2); % Segement EEG
        reeg = estimate_rEEG(dat1, fs); % Estimate the rEEG signal
        ...
        [Pxx, f] = pwelch(dat1, hamming(elen/8), elen/16, elen, fs); % Estimate the PSD
        ...
        fval3(kk2,:) = ap./sum(ap);
        ife = diff(unwrap(angle(hilbert(dat1))))/(2*pi)*fs; % Estimate the instantaneous frequency
        fval4(kk2,:) = quantile(ife, [0.25 0.5 0.75]);
        fval5(kk2) = Higuchi1Dn(resample(dat, 64, fs)); % resample for speed: Fractal dimension estimate
...
for kk1 = 1:block_no2
    ...
    dat1 = dat(r1:r2,1);
    dat2 = dat(r1:r2,2);
    fv1(kk1, 1) = getASI([dat1 dat2], fs); % Activation Synchrony Index
    fv1(kk1, 2) = corr(abs(hilbert(dat1)), abs(hilbert(dat2))); % Correlate amplitude envelopes
    fv1(kk1, 3) = rbsi(dat1', dat2', fs); % revised brain symmtery calculations
